 ...
 * @returns {{ surname: string, name: string, middlename: string, contacts: object[] }} Объект клиента
 */
 ...
 * @returns {{ id: string, surname: string, name: string, middlename: string, contacts: object[] }[]} Массив клиентов
 */
 ...
 * @returns {{ id: string, surname: string, name: string, middlename: string, contacts: object[], createdAt: string, updatedAt: string }} Объект клиента
 */
 ...
 * @param {{ surname?: string, name?: string, middlename?: string, contacts?: object[] }} data - Объект с изменяемыми данными
 * @throws {ApiError} Клиент с таким ID не найден (statusCode 404)
 * @throws {ApiError} Некорректные данные в аргументе (statusCode 422)
 * @returns {{ id: string, surname: string, name: string, middlename: string, contacts: object[], createdAt: string, updatedAt: string }} Объект клиента
 */
...
module.exports = createServer(async (req, res) => {
  ...
  .on('listening', () => {
    if (process.env.NODE_ENV !== 'test') {
      ...
      console.log(`POST ${URI_PREFIX} - создать клиента, в теле запроса нужно передать объект { surname: string, name: string, middlename?: string, contacts?: object[] }`);
      ...
      console.log(`PATCH ${URI_PREFIX}/{id} - изменить клиента с ID, в теле запроса нужно передать объект { surname?: string, name?: string, middlename?: string, contacts?: object[] }`);
      console.log(`\tcontacts - массив объектов контактов вида { type: string, value: string }`);
